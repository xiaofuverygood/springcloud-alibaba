...
import com.springcloud.alibaba.api.RemoteAccountService;
import com.springcloud.alibaba.service.OrderService;
import com.springcloud.alibaba.api.RemoteStorageService;
...
public class OrderServiceImpl implements OrderService {
	...
	public void creat(Order order) {
		...
		remoteAccountService.decrease(order.getUserId(),order.getMoney());
		log.info("订单微服务开始调用库存做扣减结束");

		//4.修改订单状态，1代表已完成，0代表未完成
		log.info("修改订单状态开始");
		orderDao.update(orderId,0);
		log.info("修改订单状态完成");
